...
class UnitHistoricInfo:
    ...
    def __init__(self, env):
        ...
        self.last_pos_x_1st_moment = 0
        self.last_pos_y_1st_moment = 0
        self.last_health_1st_moment = 0
        self.last_shield_1st_moment = 0

        # self.last_action_node = np.zeros(self.env.n_node_features)
...
class StarCraft2Env(MultiAgentEnv):
    ...
    def __init__(
            ...
            debug=False,
            position_scaling_factor=30,
            health_scaling_factor1 = 5,
            health_scaling_factor2 = 3.5,
            health_scaling_factor3 = 1,
            health_scaling_factor4 = 1
    ):
        """
        ...
        self.health_scaling_factor1 = health_scaling_factor1
        self.health_scaling_factor2 = health_scaling_factor2
        self.health_scaling_factor3 = health_scaling_factor3
        self.health_scaling_factor4 = health_scaling_factor4

        self.max_reward = (
                self.n_enemies * self.reward_death_value + self.reward_win
        )
        # print("확인", self.n_enemies, self.reward_death_value, self.reward_win)
        ...
        self.historic_infos_agents = {}
        self.historic_infos_enemies = {}
    ...
    def reset(self):
        ...
        for a_id, _ in self.agents.items():
            unit = self.get_unit_by_id(a_id)
            unit_types.append(unit.unit_type)
            unithistoricinfo = UnitHistoricInfo(env=self)
            self.historic_infos_agents[a_id] = unithistoricinfo

        for e_id, _ in self.enemies.items():
            unit = list(self.enemies.items())[e_id][1]
            # print(unit)
            unit_types.append(unit.unit_type)
            unithistoricinfo = UnitHistoricInfo(env=self)
            self.historic_infos_enemies[e_id] = unithistoricinfo
        # print(unit_types)
    ...
    def step(self, actions):
        ...
        self.reward = reward
        # if terminated == True:
        #     print("reward /= self.max_reward / self.reward_scale_rate", self.max_reward, self.reward_scale_rate)
        # print("episode step", self._episode_steps)
        # print("에피소드", self._episode_count, "step", self._episode_steps,"reward", self.reward)
    ...
    def get_enemy_feature(self):
        enemy_feature = np.array([self.get_node_feature(node_id) for node_id in range(self.n_agents,
                                                                                      self.n_agents + self.n_enemies)])  # shape : num_enemy, feature_dim
        return enemy_feature

    def get_ally_feature(self):
        ally_feature = np.array([self.get_node_feature(node_id) for node_id in range(self.n_agents)])
        return ally_feature
    ...
    def get_enemy_visibility_matrix(self):
        arr = np.zeros((self.n_agents, self.n_enemies), dtype=np.bool)
        for agent_id in range(self.n_agents):
            current_agent = self.get_unit_by_id(agent_id)
            if current_agent.health > 0:  # it agent not dead
                x = current_agent.pos.x
    ...
    def get_ally_visibility_matrix(self):
        ...
        are visible to each agent.
        """
        arr = np.zeros((self.n_agents, self.n_agents), dtype=np.bool)
    ...
    def get_node_feature(self, node_id):
        ...
        if node_id < self.n_agents:
            ...
            unithistoricinfo = self.historic_infos_enemies[node_id]
        ...
        # print(str(unit.health_max)+str(unit.shield_max)+str(unit.radius),  unit_type_feats, unit.alliance, alliance_feats)
    ...
    def get_obs_agent(self, agent_id):
        ...
        if unit.health > 0:  # otherwise dead, return all zeros
            ...
            if self.obs_pathing_grid:  # False
                move_feats[
                ind: ind + self.n_obs_pathing  # noqa
                ] = self.get_surrounding_pathing(unit)
                ind += self.n_obs_pathing

            if self.obs_terrain_height:  # False
    ...
    def get_obs_own_feats_size(self):
        ...
        if self.obs_own_health:  # True
            own_feats += 1 + self.shield_bits_ally
        if self.obs_timestep_number:  # False
            own_feats += 1
    ...
    def update_units(self):
        ...
        if n_ally_alive == 0 and n_enemy_alive == 0:
            return 0
        # print("명수 체크", n_ally_alive, n_enemy_alive, len(self.agents), len(self.enemies))
        return None
    ...
    def get_env_info(self):
        ...
        env_info["node_features"] = self.n_node_features
